...
def test_add_load_profile():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        p = ExecutionProfile(name="As deploy-user", run_as_user="deploy-user",
                             working_dir="/srv/work", description="dev")
        config.add_profile(p)
        loaded = config.load_profiles()
        assert len(loaded) == 1
        assert loaded[0].name == "As deploy-user"
        assert loaded[0].run_as_user == "deploy-user"
        assert loaded[0].working_dir == "/srv/work"
...
def test_build_ssh_command_with_working_dir():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        executor = CommandExecutorPro(config)
