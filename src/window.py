...
from feature_gate import is_pro_active, get_trial_info, get_license_info, FREE_BUTTON_LIMIT, PRO_INFO_URL, PRO_BUY_URL, make_executor
from i18n import _


def _open_browser(url: str) -> None:
    """Open a URL in the user's web browser, working around xdg-open
    silently lying about success on some desktops (notably Linux Mint).

    Order of attempts:
      1. $BROWSER env var (user override)
      2. x-www-browser (Debian/Mint alternatives system — points to the
         user's selected default)
      3. Common browser binaries via shutil.which
      4. Gio.AppInfo.launch_default_for_uri (last resort — relies on
         portal/gvfs which is the broken layer on affected systems)

    xdg-open is intentionally excluded — on Linux Mint it returns 0
    without opening anything, which makes it indistinguishable from
    success.
    """
    if not url:
        return
...
class RemotexWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.toolbar_view.add_top_bar(self._trial_banner)
        ...
        GLib.idle_add(self._check_trial_state)
    ...
    def _pick_machine_and_run(self, tile: ButtonTile, button: CommandButton):
        ...
        if len(options) > 1:
            if not is_pro_active():
                # Free build: no machine picker; the executor will report SSH-Pro errors.
                self._run_button(tile, button, options[0][0])
                return
            from pro.dialogs.machine_picker_dialog import show_machine_picker
            show_machine_picker(
                self, options,
    ...
    def show_machines_dialog(self):
        if not is_pro_active():
            self._show_pro_limit_dialog(
                _("Pro feature"),
                _("SSH machines are a Pro feature. Upgrade to RemoteX Pro to manage remote machines."),
            )
            return
        from pro.dialogs.machines_list_dialog import show_machines_list
    ...
    def _build_trial_banner(self) -> Adw.Banner:
        banner = Adw.Banner()
        banner.set_button_label(_("Upgrade"))
        banner.connect("button-clicked", lambda _: self._open_url(PRO_BUY_URL))
        return banner

    def _check_trial_state(self):
        """Show/update the trial banner and emit toasts for the trial lifecycle."""
        try:
            license_info = get_license_info()
            trial_info = get_trial_info()

            if trial_info['active'] and not license_info['active']:
                days = trial_info['days_remaining']
                self._trial_banner.set_title(
                    _("Pro trial: {days} day(s) remaining").format(days=days)
                )
                if not self._trial_banner_dismissed:
                    self._trial_banner.set_revealed(True)
                if days <= 3:
                    self.show_toast(
                        _("Trial expires in {days} day(s) — upgrade to keep Pro access").format(days=days)
                    )
            else:
                self._trial_banner.set_revealed(False)
                if trial_info['started_at'] and not trial_info['active'] and not license_info['active']:
                    self.show_toast(_("Pro trial ended — free tier limits now apply"))
        except Exception:
            pass
        return False
