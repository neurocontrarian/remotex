...
# Suppress GLib NULL-pointer assertion warnings that Adwaita/PyGObject surfaces
# as Python RuntimeWarnings during do_startup() (font/portal detection).
# These come from internal GLib defensive checks — the app handles them
# gracefully with fallbacks and they do not affect functionality.
warnings.filterwarnings('ignore', message=r'g_\w+:.*assertion.*NULL')
...
class RemotexApplication(Adw.Application):
    ...
    def _migrate_settings(self):
        ...
        HOW TO ADD A MIGRATION:
          ...
          3. Add a one-line comment next to the new `if v < N:` block
             explaining what the migration does and why.
    ...
    def _schedule_license_revalidation(self):
        ...
        from feature_gate import revalidate_license
    ...
    def _on_manage_machines(self, action, param):
        from feature_gate import is_pro_active
        win = self.props.active_window
        if not win:
            return
    ...
    def _on_quit(self, action, param):
        win = self.props.active_window
        if win and hasattr(win, '_save_window_state'):
