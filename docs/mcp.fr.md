...
Certains modèles (surtout les plus petits, en local) n'enchaînent pas automatiquement les appels d'outils lorsqu'ils ne savent pas comment procéder. Ajouter cette ligne au prompt système du modèle résout ce problème :
...
=== "Open WebUI"
    ...
    Dans Open WebUI, allez dans **Admin Panel → Settings → System prompt** et ajoutez la ligne ci-dessus (ou collez-la dans le champ de prompt système des paramètres du modèle dans le chat).

=== "Claude Desktop"

    Claude Desktop n'expose pas de champ de prompt système. La description du tool `help` est suffisamment explicite pour Claude — aucun prompt supplémentaire n'est nécessaire.

=== "Autres clients"

    Ajoutez la ligne dans le champ de prompt système ou d'instructions que votre client expose avant le début de la conversation.
