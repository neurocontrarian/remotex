...
Commandeck est exploité par **neurocontrarian**, situé au Québec, Canada. Le traitement des paiements est assuré par [LemonSqueezy](https://www.lemonsqueezy.com), agissant en tant que Merchant of Record — ils sont responsables de toutes les données de paiement client.
...
Commandeck collecte un minimum de données, strictement nécessaires à la validation de licence.
...
Commandeck stocke les fichiers suivants dans `~/.config/commandeck/` :
...
La validation de licence est gérée par [LemonSqueezy](https://www.lemonsqueezy.com). Leur politique de confidentialité s'applique aux données qu'ils traitent. Les appels de validation sont effectués lors de l'activation et au maximum une fois toutes les 30 jours lorsque l'application est en cours d'exécution. L'application fonctionne entièrement hors ligne entre ces vérifications — seuls votre clé de licence et l'identifiant de l'appareil sont transmis.
