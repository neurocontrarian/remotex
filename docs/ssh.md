...
Commandeck connects to remote servers using SSH key authentication. No passwords are ever stored.
...
| Field | Description |
|-------|-------------|
| **Name** | Display name shown in Commandeck (e.g. "Plex Server") |
...
If you don't have an SSH key pair yet, Commandeck can generate one and copy it to the server:
...
Click **Test** in the machine dialog. Commandeck runs `echo commandeck-ok` on the remote host and reports success or the error message.
...
In terminal mode you can fill in **Run as user** — Commandeck prepends `sudo -u <user>` on the remote command. Useful for running commands as a service account (e.g. `www-data`, `postgres`).
